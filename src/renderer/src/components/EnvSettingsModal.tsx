...
export function EnvSettingsModal({
  ...
  const filteredVariables = activeProfile
    ? Object.entries(activeProfile.variables).filter(
        ([key, val]) =>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/50">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">
              Manage environment variables and grouped profiles.
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="w-full md:w-64 border-r border-slate-800 bg-slate-900/30 overflow-y-auto flex-shrink-0 flex flex-col">
                  ...
                  {data.profiles.map((p) => (
                    ...
                    >
                      <div className="flex items-center gap-2 overflow-hidden">
                        ...
                        />
                        <span
                          className={`text-sm truncate font-medium ${activeProfileId === p.id ? 'text-white' : 'text-slate-300'}`}
                        >
                          {p.name}
                        </span>
                      </div>
                  ...
                  <div className="bg-slate-800 rounded-xl p-3 border border-slate-700 shadow-xl animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold py-1.5 rounded-lg transition-colors"
                      >
                        Create
                      </button>
                      <button
                        onClick={() => setIsCreatingProfile(false)}
                        className="flex-1 bg-slate-700 hover:bg-slate-600 text-white text-xs font-bold py-1.5 rounded-lg transition-colors"
                      >
                        Cancel
                      </button>
                    </div>
              ...
              <div className="p-4 mt-auto">
                <div className="text-xs text-slate-500 leading-relaxed bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
                  <span className="text-cyan-400 font-medium">Tip:</span> The active profile's
                  variables will be automatically written to the{' '}
                  <code className="bg-slate-900 px-1 py-0.5 rounded text-slate-300">.env</code> file
                  when you save.
                </div>
              </div>
            </div>
              ...
              {activeProfile ? (
                    ...
                    <div className="flex-1 flex flex-col gap-4">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="flex-1 min-w-0">
                          ...
                          <select
                            value={
                              activeProfile.envPath ||
                              (discoveredFiles.length > 0 ? discoveredFiles[0] : '')
                            }
                            onChange={(e) =>
                              handleProfileEnvPathChange(activeProfile.id, e.target.value)
                            }
                  ...
                  <div className="flex-1 overflow-y-auto p-4 max-h-[60vh] custom-scroll">
                    {filteredVariables.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-slate-500 p-8 space-y-4">
                        ...
                        <button
                          onClick={addNewVariable}
                          className="text-cyan-500 hover:text-cyan-400 font-medium text-sm transition-colors"
                        >
                          Add a new variable
                        </button>
                      </div>
                        ...
                        {filteredVariables.map(([key, val]) => (
                          <div key={key} className="flex items-start gap-2 group">
                            ...
                            <div className="flex-1 flex items-start relative">
                              <span className="absolute left-3 top-2.5 text-slate-500 font-mono text-sm select-none">
                                =
                              </span>
                              <textarea
                                value={val}
                ...
                <div className="flex-1 flex items-center justify-center text-slate-500 p-12 text-center flex-col gap-4">
                  ...
                  <p className="text-sm max-w-sm mx-auto">
                    Profiles let you quickly switch between different sets of environment variables.
                  </p>
                </div>
              )}
            </div>
