      ...
      <div className="w-full max-w-md rounded-2xl bg-slate-900 border border-slate-700/50 shadow-2xl p-6 relative overflow-hidden">
        ...
        <div className="space-y-4">
          <div className="text-sm text-gray-500 dark:text-gray-400">
            Select a branch to checkout for{' '}
            <span className="font-medium text-gray-900 dark:text-gray-200">{serviceName}</span>
          </div>
            ...
            {isLoading ? (
              ...
              <div className="max-h-60 overflow-y-auto w-full custom-scrollbar">
                {filteredBranches.length === 0 ? (
                  <div className="py-8 text-center text-sm text-slate-500">No branches found</div>
                ) : (
                  <ul className="divide-y divide-slate-800/40">
                    {filteredBranches.map((branch) => (
                      <li key={branch}>
                        ...
                        >
                          <GitBranch
                            className={`h-4 w-4 ${currentBranch === branch ? 'text-indigo-400' : 'text-slate-600 group-hover:text-indigo-400 transition-colors'}`}
                          />
