...
export const AiChatbot: React.FC<AiChatbotProps> = ({
  ...
  const handleAutoShell = async (command: string, key?: string) => {
    ...
    try {
      ...
      if (key)
        setExecutionResults((prev) => ({ ...prev, [key]: { success: false, error: e.message } }))
    } finally {
      setShellExecuting(null)
    }
  }
    ...
    if (codeMatch && codeMatch[1]) {
      try {
        await onExecuteAction(
          type === 'CREATE_FILE' ? 'create-file' : 'fix-file',
  ...
  const renderContent = (content: string) => {
    ...
    while ((match = unifiedRegex.exec(content)) !== null) {
      ...
      if (type === 'SHELL') {
        ...
        const codeContentMatch = content
          .substring(match.index + tag.length)
          .match(/```(?:\w+)?\n([\s\S]*?)```/)

        parts.push(
          <div
            key={`file-action-${match.index}`}
            className={cn(
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">
                  {type === 'CREATE_FILE' ? 'New File' : 'Suggested Fix'}
                </span>
              </div>
              {settings?.autoPilot && !result && (
              ...
              {result && (
                <span
                  className={cn(
                    'text-[10px] font-bold uppercase',
                    result.success ? 'text-purple-500' : 'text-red-500'
                  )}
                >
            ...
            {!settings?.autoPilot && !result && (
              <button
                onClick={() =>
                  handleAutoFile(type, filePath, content, match.index + tag.length, actionKey)
                }
        ...
        if (codeContentMatch) {
          unifiedRegex.lastIndex =
            match.index + tag.length + (codeContentMatch.index || 0) + codeContentMatch[0].length
        }
      } else {
    ...
    if (lastIndex < content.length) {
      ...
      parts.push(
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={`text-${lastIndex}`}>
      ...
      if (line.startsWith('```')) {
        if (inCodeBlock) {
          elements.push(
            <pre
              key={`code-${i}`}
              className="my-3 p-4 bg-slate-950 border border-white/5 rounded-xl text-[11px] font-mono overflow-x-auto text-cyan-300 shadow-inner"
            >
        ...
        processedLine = processedLine.split(/(`[^`]+`)/g).map((part: string, pi: number) => {
          if (part.startsWith('`') && part.endsWith('`')) {
            return (
              <code
                key={pi}
                className="px-1.5 py-0.5 bg-white/10 rounded font-mono text-purple-300"
              >
                {part.slice(1, -1)}
              </code>
            )
            ...
            >
              ...
              <div className="flex items-center gap-1">
                ...
                >
                  ...
                  {!isMinimized && !isHistoryOpen && (
                    <span className="text-[10px] font-black uppercase pr-1">New Chat</span>
                  )}
                </button>
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-50">
                      ...
                      <div className="max-w-[200px]">
                        ...
                        <p className="text-[10px] text-slate-500 mt-1">
                          Ask me about your services, logs, or branch status.
                        </p>
                      </div>
                    </div>
                  )}
