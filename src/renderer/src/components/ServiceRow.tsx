  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            {gitStatus && (gitStatus.ahead > 0 || gitStatus.behind > 0) && (
              <div className="flex gap-1 shrink-0">
                {gitStatus.ahead > 0 && (
                  <span className="text-[9px] font-bold text-green-500">+{gitStatus.ahead}</span>
                )}
                {gitStatus.behind > 0 && (
                  <span className="text-[9px] font-bold text-red-500">-{gitStatus.behind}</span>
                )}
              </div>
            )}
          </div>
      ...
      <div className="w-[140px] shrink-0 flex gap-2">
        <button
          onClick={() =>
            onToggle(path, status === 'running' && mode === 'dev' ? 'stop' : 'start', 'dev')
          }
        ...
        <button
          onClick={() =>
            onToggle(path, status === 'running' && mode === 'prod' ? 'stop' : 'start', 'prod')
          }
          ...
          {menuOpen &&
            createPortal(
              ...
              >
                ...
                >
                  <Archive className="h-4 w-4 text-slate-500 group-hover:text-amber-400" /> Archive
                  Channel
                </button>
