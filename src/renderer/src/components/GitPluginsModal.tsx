...
import { X, Globe, Plus, Trash2, Check, AlertCircle, Github, Database, Loader2, Link } from "lucide-react";
            ...
            <div className="w-full max-w-xl bg-slate-900 border border-slate-700/60 rounded-2xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
                ...
                <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/50">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-black text-white tracking-tight">Git <span className="text-cyan-400">Plugins</span></h2>
                            <p className="text-slate-400 text-sm font-medium">Connect to GitHub, Oracle VBS, and more</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                            ...
                            <div className="flex items-center gap-4">
                                <div className="h-10 w-10 rounded-lg bg-slate-700/50 flex items-center justify-center overflow-hidden">
                                    {conn.avatarUrl ? (
                                        ...
                                        conn.providerId === 'github' ? <Github className="h-5 w-5 text-white" /> : <Database className="h-5 w-5 text-orange-400" />
                                    )}
                                </div>
                                <div>
                                    <h4 className="font-bold text-white flex items-center gap-2">
                                        {conn.name}
                                        <span className={`text-[9px] px-1.5 py-0.5 rounded uppercase tracking-widest ${conn.providerId === 'github' ? 'bg-slate-700 text-slate-300' : 'bg-orange-500/20 text-orange-400'}`}>
                                            {conn.providerId}
                                        </span>
                                    </h4>
                        ...
                        <div className="p-6 bg-slate-800/40 border-2 border-cyan-500/30 rounded-2xl space-y-4 animate-in zoom-in-95 duration-200">
                            ...
                            <div className="space-y-4">
                                <div className="space-y-1.5">
                                    <label className="text-xs font-bold text-slate-400 uppercase px-1">Display Name (Optional)</label>
                                    <input
                                        type="text"
                                        value={newConn.name}
                                ...
                                {providerType === 'oracle-vbs' && (
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-slate-400 uppercase px-1">Base URL</label>
                                        <input
                                            type="text"
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-xs font-bold text-slate-400 uppercase px-1">Personal Access Token (PAT)</label>
                                    <input
                                        type="password"
                                        value={newConn.token}
                                    ...
                                    />
                                    <p className="text-[10px] text-slate-500 px-1 italic">Vantage stores this locally and securely for API access.</p>
                                </div>
                            </div>
