            ...
            <div className="sticky top-0 z-20 mb-6 animate-in slide-in-from-top-4 duration-300">
              <div
                className={cn(
                  ...
                  diagnosis.error?.includes('Optimal') ||
                    diagnosis.error === 'System Health: Optimal'
                    ...
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-slate-950/50 border border-slate-800">
                      <div className="flex-1 font-mono text-xs text-purple-400">
                        ...
                        <span className="text-slate-300">
                          {diagnosis.fixCommand === 'stop'
                            ? 'Restart Service'
                            : `Run "${diagnosis.fixCommand}"`}
                        </span>
                      </div>
                      <button
                      ...
                      >
                        {isExecuting ? (
                          <RefreshCw className="h-3 w-3 animate-spin mr-2inline-block" />
                        ) : null}
          ...
          {filteredLogs.length === 0 ? (
            ...
            <div className="space-y-1">
              {filteredLogs.map((log, index) => (
                <div
                  key={index}
                  className="group break-all hover:bg-white/5 px-2 py-0.5 rounded transition-colors whitespace-pre-wrap flex gap-4"
                >
                  <span className="text-slate-700 select-none min-w-[32px] text-right">
                    {index + 1}
                  </span>
