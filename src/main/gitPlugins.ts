...
export class GitPluginManager {
    ...
    async connectGitHub(token: string, name: string): Promise<{ success: boolean; error?: string; connection?: GitProviderConnection }> {
        try {
    ...
    async connectOracleVBS(token: string, name: string, baseUrl: string): Promise<{ success: boolean; error?: string; connection?: GitProviderConnection }> {
        try {
            const connection: GitProviderConnection = {
                id: uuidv4(),
