...
export function setupIpcHandlers() {
  ...
  ipcMain.handle('reset-app', async () => {
    const result = await dialog.showMessageBox({
      ...
      detail:
        'This will clear all workbenches, clusters, and saved settings. This action cannot be undone.'
    })
  ...
  ipcMain.handle('select-workbench', async () => {
    ...
    if (!result.canceled && result.filePaths.length > 0) {
      ...
      if (config.workbenchPath && config.workbenches.length === 0) {
        config.workbenches.push({
          id: require('uuid').v4(),
          path: config.workbenchPath,
          name: path.basename(config.workbenchPath)
        })
  ...
  ipcMain.handle('export-config', async () => {
    ...
    for (const file of files) {
      ...
      if (fs.existsSync(p)) {
        try {
          bundle.data[file] = JSON.parse(fs.readFileSync(p, 'utf8'))
        } catch (e) {}
      }
    }
  ...
  ipcMain.handle('import-config', async () => {
    ...
    try {
      ...
      const confirm = await dialog.showMessageBox({
        ...
        message:
          'All current settings, workbenches, and clusters will be overwritten. The app will relaunch to apply changes.',
        defaultId: 0
      })
  ...
  ipcMain.handle(
    ...
    async (
      _,
      data: { mode: string; settings: any; messages: any[]; customSystemPrompt?: string }
    ) => {
      ...
      if (mode === 'cloud') {
        ...
        try {
          if (cloudProvider === 'openai') {
            ...
            if (!res.ok) {
              throw new Error(
                result.error?.message || `OpenAI Error ${res.status}: ${res.statusText}`
              )
            }
          ...
          if (cloudProvider === 'gemini') {
            ...
            const combinedSystemMsg = [defaultSystemMsg, customSystemPrompt]
              .filter(Boolean)
              .join('\n\n')
            ...
            if (combinedSystemMsg) {
              contents.push({
                role: 'user',
                parts: [{ text: `[System context]: ${combinedSystemMsg}` }]
              })
              contents.push({
                role: 'model',
                parts: [
                  {
                    text: 'Understood. I have initialized your custom assistant personality and am ready to help.'
                  }
                ]
              })
            }
            ...
            const cleanModel = cloudModel.startsWith('models/')
              ? cloudModel.replace('models/', '')
              : cloudModel
            ...
            if (!res.ok) {
              throw new Error(
                result.error?.message || `Gemini Error ${res.status}: ${res.statusText}`
              )
            }
          ...
          if (cloudProvider === 'anthropic') {
            ...
            if (!res.ok) {
              throw new Error(
                result.error?.message || `Anthropic Error ${res.status}: ${res.statusText}`
              )
            }
  ...
  ipcMain.handle('get-services', async (_, workbenchPath: string, forceRefresh = false) => {
    ...
    const currentEntries = fs
      ...
      .filter(
        (entry) =>
          entry.isDirectory() &&
          entry.name !== 'service-dashboard' &&
          entry.name !== 'service-dashboard-desktop' &&
          !entry.name.startsWith('.')
      )
      ...
      try {
        ...
        if (JSON.stringify(currentEntries) === JSON.stringify(cachedEntries)) {
          ...
          const mergedServices = await Promise.all(
            cachedData.services.map(async (svc: any) => {
              ...
              if (serviceEnvs && serviceEnvs.active && serviceEnvs.profiles) {
                envProfiles = serviceEnvs.profiles.map((p: any) => ({
                  id: p.id,
                  name: p.name,
                  color: p.color
                }))
  ...
  ipcMain.handle('restore-service', async (_, { workbenchPath, serviceName }) => {
    ...
    if (fs.existsSync(destPath)) {
      throw new Error(
        `A service with name "${serviceName}" already exists in the workbench. Please rename or delete it first.`
      )
    }
  ...
  ipcMain.handle(
    'control-service',
    async (_, { path: servicePath, action, port, mode, customCommand }) => {
      if (action === 'start') {
  ...
  class ClusterOrchestrator {
    ...
    private currentSessionServices: string[] = []
    private startedDuringSession = new Set<string>()
    private currentServiceStatuses = new Map<string, string>()
    ...
    async runCluster(
      ...
      this.startedDuringSession.clear()

      const dependencies = group.dependencies || {}
      const pipelines = group.pipelines || {}
      const servicePaths = group.servicePaths || []
      this.currentSessionServices = servicePaths

      // Map of paths to service metadata
      const serviceMap = new Map(allServices.map((s) => [s.path, s]))

      // Keep track of which services are done
      const completed = new Set<string>()
      this.currentServiceStatuses.clear()

      // Initialize all as queued so they show up immediately in UI
      servicePaths.forEach((p) => this.currentServiceStatuses.set(p, 'queued'))

      const updateUI = () => {
        if (this.mainWindow && !this.mainWindow.isDestroyed()) {
          this.mainWindow.webContents.send('orchestration-update', {
            groupId: group.id,
            statuses: Object.fromEntries(this.currentServiceStatuses)
          })
        }
      }
      ...
      const runService = async (servicePath: string) => {
        if (completed.has(servicePath)) return

        // Wait for parents
        const parents = dependencies[servicePath] || []
        if (parents.some((p) => !completed.has(p))) {
          this.currentServiceStatuses.set(servicePath, 'waiting')
          updateUI()
        }
        for (const parent of parents) {
          while (!completed.has(parent)) {
            if (this.abortController.signal.aborted) return

            const parentStatus = this.currentServiceStatuses.get(parent)
            if (parentStatus === 'error' || parentStatus === 'aborted') {
              this.currentServiceStatuses.set(servicePath, 'aborted')
              updateUI()
              return
            }
        ...
        const svc = serviceMap.get(servicePath)
        if (!svc) return

        const pipeline = pipelines[servicePath] || []
        this.currentServiceStatuses.set(servicePath, 'starting')
        updateUI()

        try {
          // Pre-check port conflict
          if (svc.port) {
            const inUse = await processManager.isPortInUse(svc.port)
            if (inUse) {
              this.currentServiceStatuses.set(servicePath, 'port-conflict')
              updateUI()
              throw new Error(`Port ${svc.port} already in use`)
            }
          }
          ...
          for (const step of pipeline) {
            ...
            const command = step.command.replace(/\$PORT/g, svc.port?.toString() || '')

            // Start the process for this step
            this.startedDuringSession.add(servicePath)
            this.currentServiceStatuses.set(servicePath, step.name || 'executing')
            updateUI()
            processManager.injectOrchestrationLog(servicePath, `Starting step: ${step.name || 'executing'}`)
            processManager.injectOrchestrationLog(servicePath, `Command: ${command}`)
            
            const svcMode = group.serviceModes?.[servicePath] || 'dev'
            await processManager.startService(servicePath, command, svc.port, svcMode, 'starting')

            // Helper to watch for process crashes while waiting
            const waitWithMonitor = async (waitFn: () => Promise<void>) => {
              ...
              return new Promise<void>((resolve, reject) => {
                ...
                monitorInterval = setInterval(async () => {
                  if (resolved) return
                  const statusInfo = await processManager.getServiceStatus(servicePath)
                  if (
                    statusInfo.status === 'error' ||
                    statusInfo.status === 'build-error' ||
                    statusInfo.status === 'install-error' ||
                    (statusInfo.status === 'stopped' && !step.isTransient)
                  ) {
                    cleanup()
                    reject(
                      new Error(
                        `Service crashed during step "${step.name || 'executing'}": ${statusInfo.status}`
                      )
                    )
                  }
                }, 500)
            ...
            if (step.successPattern) {
              processManager.injectOrchestrationLog(servicePath, `Waiting for log pattern: ${step.successPattern}`)
              try {
                await waitWithMonitor(() =>
                  processManager.waitForLog(
                    servicePath,
                    step.successPattern!,
                    step.timeout || 0,
                ...
                // Trigger hanging popup via IPC
                this.mainWindow?.webContents.send('startup-hanging', {
                  servicePath,
                  serviceName: svc.name,
                  logPattern: step.successPattern,
                  logs: processManager.getLogs(servicePath).slice(-20)
                })
                // Wait for user decision
                await processManager.waitForLog(
                  servicePath,
                  '__MANUAL_READY__',
                  3600000,
                  this.abortController.signal
              ...
              processManager.injectOrchestrationLog(servicePath, `Step successful: Log pattern matched.`)
            } else if (step.isWaitPort && svc.port) {
              processManager.injectOrchestrationLog(servicePath, `Waiting for port ${svc.port}...`)
              await waitWithMonitor(() =>
                processManager.waitForPort(
                  svc.port!,
                  step.timeout || 0,
                  this.abortController.signal
                )
              )
              processManager.injectOrchestrationLog(servicePath, `Step successful: Port ${svc.port} is active.`)
            }

            if (this.abortController.signal.aborted) return

            // If it's a transient command (like install), we log that it finished but let the user decide when to stop.
            if (step.isTransient) {
              processManager.injectOrchestrationLog(servicePath, `[ACTION REQUIRED] Transient step '${step.name}' finished. Service remains running for your review.`)
            }
          }

          processManager.injectOrchestrationLog(servicePath, `Orchestration complete: Service is READY.`)
          processManager.setStatus(servicePath, 'ready')
          this.currentServiceStatuses.set(servicePath, 'ready')
          completed.add(servicePath)
          updateUI()
        } catch (err: any) {
          if (err.message === 'Aborted') return

          this.currentServiceStatuses.set(servicePath, 'error')
          updateUI()
          console.error(`Orchestration failed for ${servicePath}:`, err)
          throw err // Propagate to runCluster
        }
      }

      try {
        // Run all services. The runService logic handles dependency waiting internally.
        await Promise.all(servicePaths.map((path) => runService(path)))
      } catch (err) {
        console.error('Cluster execution halted due to error')
        // Automatically abort the rest of the cluster to ensure consistent state
        // but tell it to be gentle with already-errored services
        await this.abort('Cluster execution failed')
      } finally {
        this.running = false
      }
    }
    ...
    async abort(reason = 'User requested abort') {
      if (!this.running && reason === 'User requested abort') return

      this.abortController.abort()

      // Mark all non-ready services as aborted
      for (const path of this.currentSessionServices) {
        const status = this.currentServiceStatuses.get(path)
        if (status !== 'ready' && status !== 'error' && status !== 'port-conflict') {
          this.currentServiceStatuses.set(path, 'aborted')
        }
      }
      ...
      // Kill all processes we started during this session
      const killPromises = Array.from(this.startedDuringSession).map(async (path) => {
        const status = this.currentServiceStatuses.get(path)
        // If it already errored, don't try to kill it again (it might have crashed and gone)
        // but if it's still "starting" or in a custom step, we should stop it
      if (reason === 'User requested abort' && status !== 'error') {
        await processManager.stopService(path, reason)
      } else if (reason !== 'User requested abort') {
        processManager.injectOrchestrationLog(path, `[NOTICE] Orchestration halted: ${reason}. Service remains running.`)
      }
      })
  ...
  ipcMain.handle('run-cluster', async (event, { workbenchId, groupId, allServices }) => {
    const p = getGroupsPath()
    if (!fs.existsSync(p)) return { success: false }
    const data = JSON.parse(fs.readFileSync(p, 'utf8'))
    const groups = data[workbenchId] || []
    ...
    orchestrator.runCluster(workbenchId, group, allServices, mainWindow)
    return { success: true }
  })

  ipcMain.handle('abort-cluster', async () => {
    await orchestrator.abort()
    return { success: true }
  })

  ipcMain.handle('mark-service-ready', (_, { servicePath }) => {
    processManager.markReady(servicePath)
    return { success: true }
  })
  ...
  ipcMain.handle('sync-env-from-disk', (_, { path: servicePath }) => {
    const envFilePath = path.join(servicePath, '.env')
    if (!fs.existsSync(envFilePath)) return { success: false, error: 'No .env file found on disk' }

    try {
      const fileContent = fs.readFileSync(envFilePath, 'utf8')
      const diskVariables = dotenv.parse(fileContent)

      const p = getEnvironmentsPath()
      let envs: any = {}
      if (fs.existsSync(p)) envs = JSON.parse(fs.readFileSync(p, 'utf8'))

      const serviceEnvs = envs[servicePath] || { active: null, profiles: [] }
      
      // If no profiles exist, create a Default one
      if (serviceEnvs.profiles.length === 0) {
        const defaultId = require('crypto').randomUUID()
        serviceEnvs.profiles.push({
          id: defaultId,
          name: 'Default',
          color: '#10b981',
          variables: diskVariables
        })
        serviceEnvs.active = defaultId
      } else {
        // Sync into the active profile or the Default profile if active is null
        const activeProfile = serviceEnvs.profiles.find((p: any) => p.id === serviceEnvs.active) || serviceEnvs.profiles[0]
        if (activeProfile) {
          activeProfile.variables = diskVariables
        }
      }

      envs[servicePath] = serviceEnvs
      fs.writeFileSync(p, JSON.stringify(envs, null, 2))
      return { success: true, variables: diskVariables }
    } catch (err: any) {
  ...
  ipcMain.handle('save-env', (_, { path: servicePath, data }) => {
    ...
    if (activeProfile && activeProfile.variables) {
      const envStr = Object.keys(activeProfile.variables)
        .map((k) => {
  ...
  ipcMain.handle('git-command', async (_, { action, path: repoPath, branch }) => {
    try {
      if (action === 'checkout')
        await gitQueue.add(() =>
          execAsync(`git checkout ${branch}`, { cwd: repoPath, env: GIT_ENV })
        )
      else if (action === 'pull')
        await gitQueue.add(() => execAsync('git pull', { cwd: repoPath, env: GIT_ENV }))
      else if (action === 'get-diff') {
        const { stdout } = await gitQueue.add(() =>
          execAsync('git diff HEAD --stat && git diff HEAD', { cwd: repoPath, env: GIT_ENV })
        )
        return { diff: stdout }
  ...
  ipcMain.handle('open-terminal', (_, cwd: string) => {
    ...
    if (osPlatform === 'win32') {
      ...
      const termCmd = fs.existsSync('/usr/bin/x-terminal-emulator')
        ? 'x-terminal-emulator'
        : 'gnome-terminal'
      ...
      child.on('close', async (code) => {
        if (code === 0 && profile) {
          try {
            ...
            await gitQueue.add(() =>
              execAsync(`git config user.name "${profile.name}"`, { cwd: targetPath, env: GIT_ENV })
            )
            await gitQueue.add(() =>
              execAsync(`git config user.email "${profile.email}"`, {
                cwd: targetPath,
                env: GIT_ENV
              })
            )
  ...
  ipcMain.handle('fs-read-file', async (_, filePath: string) => {
    if (!validatePath(filePath))
      throw new Error('Access denied: Path is outside of registered workbenches.')
  ...
  ipcMain.handle('take-snapshot', async (event) => {
    try {
      if (!event.sender) throw new Error('No sender available')
      const mainWindow = BrowserWindow.fromWebContents(event.sender)
      if (!mainWindow) throw new Error('No window available')

      const nativeImage = await mainWindow.capturePage()
      const png = nativeImage.toPNG()

      const snapsDir = path.join(app.getPath('pictures'), 'Vantage-Snaps')
      if (!fs.existsSync(snapsDir)) {
        fs.mkdirSync(snapsDir, { recursive: true })
      }

      const fileName = `vantage-snap-${new Date().getTime()}.png`
      const filePath = path.join(snapsDir, fileName)
  ...
  ipcMain.handle('save-recording', async (_, { buffer }) => {
    try {
      ...
      const fileName = `vantage-recording-${new Date().getTime()}.webm`
      const filePath = path.join(snapsDir, fileName)
